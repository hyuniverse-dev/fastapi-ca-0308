...
class UserService:
    ...
    def create_user(self, name: str, email: str, password: str) -> User:
        ...
        try:
            _user = self.user_repo.find_by_email(email) # (중복 유저 검사-1) 저장된 데이터에서 email로 유저를 찾는다. 새로 생성할 유저와 구분하기 위해 _(언더바) 사용
        except HTTPException as e:
            if e.status_code != 422: # 리소스 생성 시에 중복된 데이터 존재하면 422 코드를 사용하기로 정한다.
                raise e

        if _user:
            raise HTTPException(status_code=422, detail="이미 존재하는 이메일입니다.") # (중복 유저 검사-2) _user 가 True이면 중복된 것으로 간주하고 422 예외를 발생시킨다.
